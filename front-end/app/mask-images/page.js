...
export default function ImageInpaintingEditor() {
  ...
  const handleOriginalImageChange = useCallback((e) => {
    ...
    reader.onloadend = () => {
      ...
      img.onload = () => {
        ...
        setCanvasSize({ width: Math.round(displayWidth), height: Math.round(displayHeight) });
  ...
  useEffect(() => {
    if (originalImage && canvasRef.current && maskCanvasRef.current) {
      ...
      // Draw original image
      ctx.drawImage(originalImage.element, 0, 0, canvasSize.width, canvasSize.height);
  ...
  const undoLastStroke = () => {
    // For simplicity, this clears the entire mask
    // In a full implementation, you'd want to maintain a history stack
    clearMask();
  };

  const getMaskDataUrl = () => {
    if (!maskCanvas || !originalImageDimensions.width || !originalImageDimensions.height) return null;
    ...
    for (let i = 0; i < maskData.length; i += 4) {
      if (maskData[i + 3] > 0) { // If alpha > 0 (painted area)
    ...
    finalCtx.drawImage(tempCanvas, 0, 0, originalImageDimensions.width, originalImageDimensions.height);
  ...
  const canSubmit = originalImage && hasMask() && prompt.trim().length > 0 && !isLoading;
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-300 max-w-2xl mx-auto">
            Upload an image and paint the areas you want to edit. Our AI will regenerate those areas based on your prompt.
          </p>
        </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700">
            ...
            {!originalImage ? (
              <label className="border-2 border-dashed border-gray-600 rounded-lg p-8 flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 transition-colors">
                ...
                <p className="text-lg font-medium text-gray-300 mb-2">Click to upload your image</p>
                <p className="text-sm text-gray-500">PNG, JPG or GIF (max. {MAX_FILE_SIZE_MB}MB)</p>
              </label>
            ) : (
              <div className="bg-gray-700 rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="font-medium text-green-400">✓ Image uploaded</p>
